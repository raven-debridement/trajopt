...
int gregTests(int argc, char *argv[]) {
  int T = 26;
  ...
  // 0.778758 0.0937199 0.598627 -0.162477 0.222236 -0.346716 0.175725
  ...
  Matrix4d goal_trans;
  goal_trans = transformToMatrix(robot->GetActiveManipulator()->GetEndEffectorTransform());
  //Matrix4d goal_trans(rave_trans.trans, rave_trans.rot);
  //goal_trans = Matrix4d(robot->GetActiveManipulator()->GetEndEffectorTransform());
  ...
  cout << "Active manipulator EE transform" << endl;
  ...
  handles.push_back(viewer->PlotAxes(robot->GetActiveManipulator()->GetEndEffectorTransform(),.01));

  //robot->SetDOFValues(toDblVec(end),0,robot->GetActiveManipulator()->GetArmIndices());
  handles.push_back(viewer->PlotAxes(matrixToTransform(goal_trans),.01));
