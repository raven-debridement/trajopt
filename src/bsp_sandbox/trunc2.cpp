...
int main()
{
	...
	*/
	...
	xVar(0,0) = 0.104258; xVar(0,1) = 0.00860636;
	xVar(1,0) = 0.00860636; xVar(1,1) = 0.00646838;
	...
	double r1 = y1NewVar/y1Var, r2 = y2NewVar/y2Var;
	double cdf1 = cdf((c1 - y1Mean)/(sqrt(y1Var)));
	double cdf2 = cdf((c2 - y2Mean)/(sqrt(y2Var)));

	xNewVar = xVar + (r1 - 1.0)*(L1*~xy1Covar) + (r2 - 1.0)*(L2*~xy2Covar);
	//xNewVar = xVar + ((cdf1 + r1)*0.5 - 1.0)*(L1*~xy1Covar) + ((cdf2 + r2)*0.5 - 1.0)*(L2*~xy2Covar);
	...
	for (int i = 0; i < numSamples; ++i) {
		...
		samples[i] = sampleGaussian(xMean, xVar);
	}

	Matrix<2> sampleMean = zeros<2,1>();
