...
namespace FourLinksRobotBSP {
  ...
  void FourLinksRobotBSPProblemHelper::add_collision_term(OptProb& prob) {
    ...
    //for (int i = 0; i < T; ++i) {
      ...
      prob.addIneqConstraint(ConstraintPtr(new BeliefCollisionConstraint<FourLinksRobotBeliefFunc>(0.05, 1, rad, belief_vars.row(i), belief_func, link)));
      break;
...
int main(int argc, char *argv[]) {
  ...
  env->StopSimulation();
  env->Load(data_dir + "/three_links.env.xml");
  ...
  planner->link_lengths = Vector4d(2, 2, 2, 0);
