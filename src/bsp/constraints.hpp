...
namespace BSP {
  ...
  public:
    ...
    //cout << "getting support vertex for " << toVector(vec).transpose() << endl;
    ...
    void getAabb(const btTransform& t_w0,btVector3& aabbMin,btVector3& aabbMax) const {
      ...
      //cout << "aabbMin: " << toVector(aabbMin).transpose() << endl;
      //cout << "aabbMax: " << toVector(aabbMax).transpose() << endl;
    }
  ...
  public:
    ...
    void CheckShapeMultiCast(btCollisionShape* shape, const vector<btTransform>& tfi,
      ...
      if (btConvexShape* convex = dynamic_cast<btConvexShape*>(shape)) {
        ...
        //for (int i=0; i<tfi.size(); i++) cout << toMatrix(t0i[i]) << endl << endl;
        MultiCastHullShape* shape = new MultiCastHullShape(convex, t0i);
    ...
    virtual void CollisionsToDistanceExpressions(const vector<BeliefCollision>& collisions, Configuration& rad,
      ...
      BOOST_FOREACH(const BeliefCollision& col, collisions) {
        ...
        for (int i=0; i<col.mi.alpha.size(); i++) {
          ...
          //cout << "instance_ind: " << col.mi.instance_ind[i] << endl;
          //cout << "belief: " << belief.transpose() << endl;
          //cout << "grad: " << endl << grad << endl << endl;
          ...
          if (linkAFound) {
            ...
            //cout << "pos_jac: " << endl << pos_jac << endl << endl;
            //cout << "dist_grad: " << dist_grad.transpose() << endl;
  ...
  template< class BeliefFuncT >
  class BeliefCollisionConstraint : public IneqConstraint, public Plotter {
  public:
    typedef typename BeliefFuncT::Ptr BeliefFuncPtr;
    ...
    virtual void Plot(const DblVec& x, OR::EnvironmentBase& env, std::vector<OR::GraphHandlePtr>& handles) {
      vector<BeliefCollision> collisions;
  ...
  template< class BeliefFuncT >
  class BeliefCollisionCost : public Cost, public Plotter {
  public:
    typedef typename BeliefFuncT::Ptr BeliefFuncPtr;
