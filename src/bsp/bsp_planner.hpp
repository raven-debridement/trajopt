...
public:
	...
	void solve(boost::function<void(OptProb*, DblVec&)>& opt_callback) {
		...
		for (int i = 1; i <= n_alpha_iterations; ++i) {
			...
			//opt.merit_error_coeff_ = fmin(pow(10,(double)i),1000);
			opt.optimize();
			this->result = opt.x();
			...
			//helper->belief_func->approx_factor += 5;
		}
	...
	void simulate_executions(int nsteps) {
		cout << "Simulating execution for one timestep" << endl;
		cout << "Controls size: " << controls.size() << endl;
		assert (initialized);
		...
		for(int i = 0; i < (int)controls.size(); ++i) {
			cout << controls[i](0) << " " << controls[i](1) << endl;
		}
		...
		for (int i = 0; i < nsteps; ++i) {
			cout << "Inside execution loop" << endl;
