...
void BSPTrustRegionSQP::addAlphaDoneCallback(const Callback& cb) {
	alpha_done_callbacks_.push_back(cb);
}
...
void BSPTrustRegionSQP::callAlphaDoneCallbacks(DblVec& x) {
	for (int i = 0; i < alpha_done_callbacks_.size(); ++i) {
		alpha_done_callbacks_[i](prob_.get(), x);
	}
}
...
/*
OptStatus BSPTrustRegionSQP::optimize2() {
	...
	OptStatus retval = INVALID;
	double original_merit_error_coeff_ = merit_error_coeff_;
	double original_trust_box_size_ = trust_box_size_;

	for (int alpha_increases = 0; alpha_increases < max_alpha_increases_; ++alpha_increases) { // relaxation loop
		...
		for (int merit_increases=0; merit_increases < max_merit_coeff_increases_; ++merit_increases) { // merit adjustment loop
			...
			for (int iter=1; ;) { // sqp loop
				...
				while (trust_box_size_ >= min_trust_box_size_) {
					...
					if (approx_merit_improve / old_merit < min_approx_improve_frac_) {
						...
						adjustTrustRegion(trust_shrink_ratio_);
						LOG_INFO("shrunk trust region. new box size: %.4f",	trust_box_size_);
					} else {
						x_ = new_x;
						results_.cost_vals = new_cost_vals; //evaluateCosts(prob_->getCosts(), x_);
						results_.cnt_viols = new_cnt_viols; //evaluateConstraintViols(constraints, x_);
			...
			penaltyadjustment: {
				...
				else if (merit_increases == max_merit_coeff_increases_) {
					retval = OPT_PENALTY_ITERATION_LIMIT;
					LOG_INFO("optimization couldn't satisfy all constraints");
					goto alphaincrease;
				}
		...
		alphaincrease: {
			LOG_INFO("increasing alpha factor; solving more constrained problem");
			merit_error_coeff_ = original_merit_error_coeff_;
			trust_box_size_ = original_trust_box_size_;
			callAlphaDoneCallbacks(x_);
		}

		retval = OPT_ALPHA_ITERATION_LIMIT;
		LOG_INFO("optimization couldn't satisfy all constraints for relaxed problem");
	}
