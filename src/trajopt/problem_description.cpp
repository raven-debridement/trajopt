...
void RegisterMakers() {
	...
	CostInfo::RegisterMaker("continuous_collision",
			&ContinuousCollisionCostInfo::create);
...
BeliefRobotAndDOFPtr RADFromName(const string& name, RobotBasePtr robot) {
	if (name == "active") {
		return BeliefRobotAndDOFPtr(
				new BeliefRobotAndDOF(robot, robot->GetActiveDOFIndices(),
						robot->GetAffineDOF(), robot->GetAffineRotationAxis()));
	}
	vector<int> dof_inds;
	int affinedofs = 0;
	...
	for (int i = 0; i < components.size(); ++i) {
		...
		if (RobotBase::ManipulatorPtr manip = GetManipulatorByName(*robot,
				component)) {
			...
			PRINT_AND_THROW(
					boost::format("error in reading manip description: %s must be a manipulator, link, or 'base'")%component);
	}
	return BeliefRobotAndDOFPtr(
			new BeliefRobotAndDOF(robot, dof_inds, affinedofs, rotationaxis));
}
...
void fromJson(const Json::Value& v, CntInfoPtr& cnt) {
	...
	if (!cnt)
		PRINT_AND_THROW(
				boost::format("failed to construct constraint named %s")%type);
	cnt->fromJson(v);
...
void InitInfo::fromJson(const Json::Value& v) {
	...
	if (type_str == "stationary") {
		data = toVectorXd(gPCI->rad->GetDOFValues()).transpose().replicate(n_steps,
				1);
		...
		if (endpoint.size() != n_dof) {
			PRINT_AND_THROW(
					boost::format("wrong number of dof values in initialization. expected %i got %j")%n_dof%endpoint.size());
		}
		data = TrajArray(n_steps, n_dof);
		DblVec start = gPCI->rad->GetDOFValues();
		for (int idof = 0; idof < n_dof; ++idof) {
			data.col(idof) = VectorXd::LinSpaced(n_steps, start[idof],
					endpoint[idof]);
		}
	}
...
void ProblemConstructionInfo::fromJson(const Value& v) {
	...
	RobotBasePtr robot =
			(basic_info.robot == "") ? GetRobot(*env) :
					GetRobotByName(*env, basic_info.robot);
	if (!robot) {
	...
	gPCI = this;
	if (v.isMember("costs"))
		fromJsonArray(v["costs"], cost_infos);
	if (v.isMember("constraints"))
		fromJsonArray(v["constraints"], cnt_infos);
...
TrajOptProbPtr ConstructProblem(const ProblemConstructionInfo& pci) {
	...
	if (bi.belief_space)
		prob->m_traj_vars = VarArray(n_steps, prob->GetRAD()->GetNTheta() + n_dof,
				prob->vars_.data());
	else
	...
	if (bi.start_fixed) {
		if (pci.init_info.data.rows() > 0
				&& !allClose(toVectorXd(cur_dofvals), pci.init_info.data.row(0))) {
			PRINT_AND_THROW(
					"robot dof values don't match initialization. I don't know what you want me to use for the dof values");
		}
		for (int j = 0; j < n_dof; ++j) {
			prob->addLinearConstr(
					exprSub(AffExpr(prob->m_traj_vars(0, j)), cur_dofvals[j]), EQ);
		}
	}
	...
	if (!bi.dofs_fixed.empty()) {
		BOOST_FOREACH(const int& dof_ind, bi.dofs_fixed) {
			for (int i = 1; i < prob->GetNumSteps(); ++i) {
				prob->addLinearConstr(
						exprSub(AffExpr(prob->m_traj_vars(i, dof_ind)),
								AffExpr(prob->m_traj_vars(0, dof_ind))), EQ);
			}
		}
	}
	...
	if (bi.belief_space) {
		...
		for (int i = 0; i < n_steps - 1; ++i) {
			VarVector theta0_vars =
					prob->m_traj_vars.block(0, 0, n_steps, n_theta).row(i);
			VarVector theta1_vars =
					prob->m_traj_vars.block(0, 0, n_steps, n_theta).row(i + 1);
			VarVector u_vars =
					prob->m_traj_vars.block(0, n_theta, n_steps, n_dof).row(i);
			prob->addConstr(
					ConstraintPtr(
							new BeliefDynamicsConstraint(theta0_vars, theta1_vars, u_vars,
									prob->GetRAD())));
		}

		MatrixXd rt_Sigma0;
		if (n_dof == 3)
			rt_Sigma0 = MatrixXd::Identity(n_dof, n_dof) * 0.1;
		else
			rt_Sigma0 = MatrixXd::Identity(n_dof, n_dof) * sqrt(5);
		...
		if (bi.start_fixed) {
			for (int j = n_dof; j < n_theta; ++j) {
				prob->addLinearConstr(
						exprSub(AffExpr(prob->m_traj_vars(0, j)), init_data(0, j)), EQ);
			}
		}
...
TrajOptProb::TrajOptProb(int n_steps, BeliefRobotAndDOFPtr rad) :
		m_rad(rad) {
	DblVec lower, upper;
...
void PoseCostInfo::hatch(TrajOptProb& prob) {
	prob.addCost(
			CostPtr(
					new CartPoseCost(prob.GetVarRow(timestep), toRaveTransform(wxyz, xyz),
							rot_coeffs, pos_coeffs, prob.GetRAD(), link)));
...
void JointPosCostInfo::fromJson(const Value& v) {
	...
	if (vals.size() != n_dof) {
		PRINT_AND_THROW(
				boost::format("wrong number of dof vals. expected %i got %i")%n_dof%vals.size());
	}
	childFromJson(params, timestep, "timestep", gPCI->basic_info.n_steps - 1);
}
void JointPosCostInfo::hatch(TrajOptProb& prob) {
	prob.addCost(
			CostPtr(
					new JointPosCost(prob.GetVarRow(timestep), toVectorXd(vals),
							toVectorXd(coeffs))));
	prob.getCosts().back()->setName(name);
}
...
void PoseCntInfo::hatch(TrajOptProb& prob) {
	...
	prob.addConstr(
			ConstraintPtr(
					new CartPoseConstraint(prob.GetVarRow(timestep),
							toRaveTransform(wxyz, xyz), prob.GetRAD(), link, coeffs)));
...
void CartVelCntInfo::fromJson(const Value& v) {
	...
	FAIL_IF_FALSE(
			(first_step >= 0) && (first_step <= gPCI->basic_info.n_steps-1) && (first_step < last_step));
...
void CartVelCntInfo::hatch(TrajOptProb& prob) {
	for (int iStep = first_step; iStep < last_step; ++iStep) {
		prob.addConstr(
				ConstraintPtr(
						new CartVelConstraint(prob.GetVarRow(iStep),
								prob.GetVarRow(iStep + 1), prob.GetRAD(), link,
								distance_limit)));
...
void JointVelCostInfo::fromJson(const Value& v) {
	...
	if (coeffs.size() == 1)
		...
		PRINT_AND_THROW(
				boost::format("wrong number of coeffs. expected %i got %i")%n_dof%coeffs.size());
	}
}
...
void JointVelCostInfo::hatch(TrajOptProb& prob) {
	...
	prob.addCost(
			CostPtr(
					new JointVelCost(
							prob.GetVars().block(0, 0, prob.GetVars().m_nRow,
									prob.GetRAD()->GetDOF()), toVectorXd(coeffs))));
	prob.getCosts().back()->setName(name);
}
...
void CollisionCostInfo::fromJson(const Value& v) {
	...
	if (coeffs.size() == 1)
		...
		PRINT_AND_THROW(
				boost::format("wrong size: coeffs. expected %i got %i")%n_steps%coeffs.size());
	}
	childFromJson(params, dist_pen, "dist_pen");
	if (dist_pen.size() == 1)
		dist_pen = DblVec(n_steps, dist_pen[0]);
	else if (dist_pen.size() != n_steps) {
		PRINT_AND_THROW(
				boost::format("wrong size: dist_pen. expected %i got %i")%n_steps%dist_pen.size());
	}
}
void CollisionCostInfo::hatch(TrajOptProb& prob) {
	for (int i = 0; i < prob.GetNumSteps(); ++i) {
		prob.addCost(
				CostPtr(
						new CollisionCost(dist_pen[i], coeffs[i], prob.GetRAD(),
								prob.GetVars().rblock(i, 0, prob.GetRAD()->GetDOF()))));
		prob.getCosts().back()->setName((boost::format("%s_%i") % name % i).str());
	}
	CollisionCheckerPtr cc = CollisionChecker::GetOrCreate(*prob.GetEnv());
	cc->SetContactDistance(
			*std::max_element(dist_pen.begin(), dist_pen.end()) + .04);
}
...
void ContinuousCollisionCostInfo::fromJson(const Value& v) {
	...
	if (coeffs.size() == 1)
		...
		PRINT_AND_THROW(
				boost::format("wrong size: coeffs. expected %i got %i")%n_terms%coeffs.size());
	}
	childFromJson(params, dist_pen, "dist_pen");
	if (dist_pen.size() == 1)
		...
		PRINT_AND_THROW(
				boost::format("wrong size: dist_pen. expected %i got %i")%n_terms%dist_pen.size());
	}
}
	...
	for (int i = first_step; i < last_step; ++i) {
		prob.addCost(
				CostPtr(
						new CollisionCost(dist_pen[i], coeffs[i], prob.GetRAD(),
								prob.GetVars().rblock(i, 0, prob.GetRAD()->GetDOF()),
								prob.GetVars().rblock(i + 1, 0, prob.GetRAD()->GetDOF()))));
...
void ControlCostInfo::hatch(TrajOptProb& prob) {
	...
	prob.addCost(
			CostPtr(
					new ControlCost(
							prob.GetVars().block(0, prob.GetRAD()->GetNTheta(),
									prob.GetVars().m_nRow - 1, prob.GetRAD()->GetDOF()),
							toVectorXd(coeffs))));
}
...
void ControlCntInfo::hatch(TrajOptProb& prob) {
	...
	for (int i = 0; i < prob.GetVars().m_nRow - 1; i++) {
		for (int j = n_theta; j < n_theta + n_dof; j++) {
			...
			prob.addLinearConstr(
					exprMult(exprSub(AffExpr(prob.GetVars()(i, j)), u_min), -1), INEQ);
		}
	}
}
...
void CovarianceCostInfo::fromJson(const Value& v) {
	...
	int n_dof = gPCI->rad->GetDOF();
	if (Q_array.size() != n_dof)
		PRINT_AND_THROW("cost matrix for the covariance has wrong number of rows");
	Q = MatrixXd(n_dof, n_dof);
	for (int i = 0; i < n_dof; i++) {
		if (Q_array[i].size() != n_dof)
			PRINT_AND_THROW(
					"cost matrix for the covariance has wrong number of columns");
		DblVec row;
